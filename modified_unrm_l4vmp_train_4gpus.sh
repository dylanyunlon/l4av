...
#PBS -l select=1:gpuname=hopper:ngpus=4:ncpus=8:mpiprocs=8:mem=64000mb
...
# Function to monitor GPU usage every 10 minutes
monitor_gpu_usage() {
    local log_file="gpu_monitoring.log"
    echo "Starting GPU monitoring (every 10 minutes)..." | tee -a $log_file
    ...
    while true; do
        ...
        if ! pgrep -f "train.py" > /dev/null; then
            echo "$(date): Training process not found, stopping GPU monitoring" | tee -a $log_file
            break
        fi
        
        echo "===========================================" | tee -a $log_file
        echo "$(date): GPU Status Report" | tee -a $log_file
        echo "===========================================" | tee -a $log_file
        nvidia-smi | tee -a $log_file
        echo "" | tee -a $log_file
        
        # Also show GPU memory usage in a more compact format
        echo "GPU Memory Usage Summary:" | tee -a $log_file
        nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu,temperature.gpu --format=csv,noheader,nounits | tee -a $log_file
        echo "===========================================" | tee -a $log_file
        echo "" | tee -a $log_file
    done
}
...
colossalai run --nproc_per_node=4 --master_port=30013 train.py \
...
echo "===========================================" | tee -a gpu_monitoring.log
echo "$(date): Final GPU Status Report" | tee -a gpu_monitoring.log
echo "===========================================" | tee -a gpu_monitoring.log
nvidia-smi | tee -a gpu_monitoring.log
...
echo "GPU monitoring log saved to: gpu_monitoring.log"
