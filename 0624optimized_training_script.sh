...
case $TRAINING_MODE in
    1)
        echo "Using Standard LoRA configuration (optimized for multi-GPU)..."
        USE_LORA=True
        LORA_R=16
        LORA_ALPHA=32
        LORA_DROPOUT=0.1
        BATCH_SIZE=2  # Reduced from 4 to better utilize both GPUs
        LEARNING_RATE=2e-4
        GRAD_ACCUM=8  # Increased to maintain effective batch size
        OUTPUT_DIR="./code_llama_c2asm_lora_r16_model"
        ;;
    2)
        echo "Using High-rank LoRA configuration (optimized for multi-GPU)..."
        USE_LORA=True
        LORA_R=64
        LORA_ALPHA=128
        LORA_DROPOUT=0.1
        ...
        ;;
    3)
        echo "Using Low-rank LoRA configuration (optimized for multi-GPU)..."
        USE_LORA=True
        LORA_R=8
        LORA_ALPHA=16
        LORA_DROPOUT=0.05
        ...
        ;;
    4)
        echo "Using Full fine-tuning configuration (optimized for multi-GPU)..."
        USE_LORA=False
        LORA_R=16
        LORA_ALPHA=32
        LORA_DROPOUT=0.1
...
# Create DeepSpeed configuration with auto values to prevent conflicts
...
echo "✓ DeepSpeed configuration created with 'auto' values"
...
echo "$TRAINING_CMD"
echo ""
echo "Multi-GPU Training Information:"
echo "  Using DeepSpeed ZeRO Stage 2 for memory optimization"
echo "  GPUs: 2 x NVIDIA H100 PCIe (79.2 GB each)"
echo "  Total effective batch size: $((BATCH_SIZE * GRAD_ACCUM * 2))"
echo "  Memory per GPU should be approximately 50-60% utilized"
echo ""
...
- Use TRAINING_MODE=1,2,3,4 to select different configurations
EOF
else
    cat << EOF
...
echo ""
echo "Multi-GPU Training Notes:"
echo "- Used DeepSpeed ZeRO Stage 2 for optimal memory distribution"
echo "- Model parameters and gradients distributed across both GPUs"
echo "- Effective batch size: $((BATCH_SIZE * GRAD_ACCUM * 2)) (batch_size × grad_accum × num_gpus)"
echo ""
